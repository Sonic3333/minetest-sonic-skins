...
local S = minetest.get_translator and minetest.get_translator("sonic_skins") or
		dofile(skins.modpath .. "/intllib.lua")

skins = {
	skins = {}, list = {}, meta = {}, formspec = {},
	modpath = minetest.get_modpath("sonic_skins"),
	...
	id = 1,
	file = minetest.get_worldpath() .. "/sonic_skins.mt",
	preview = minetest.settings:get_bool("sonic_skins_preview"),
	translate = S
}
...
skins.event_CHG = function(event, player)
	...
	meta:set_string("sonic:skin", skins.skins[name])
end
...
minetest.register_on_joinplayer(function(player)
	...
	local meta = player:get_meta()
	local skin = meta:get_string("sonic:skin")

	-- do we already have a skin in player attributes?
	if skin and skin ~= "" then
		skins.skins[name] = skin

	-- otherwise use skin from sonic_skins.mt file or default if not set
...
--		end
		...
		meta:set_string("sonic:skin", skins.skins[playername])
